...
> **當前階段**: 股票 Alpha 研究層已完成，正在擴展為多資產架構
> **代碼庫起始日期**: 2026-03-22
> **總提交次數**: 44 commits
> **當前分支**: master
...
| `src/api/` | 17 | ~2,500 | REST API + WebSocket + Alpha 端點 |
...
| 分類 | 檔案 | 框架 |
|------|------|------|
| Python 單元/整合 | 38 | pytest (~6,700 LOC) |
...
**後端**: 38 檔 — 含 9 個 Alpha 測試檔 (universe, neutralize, cross_section, turnover, orthogonalize, construction, pipeline, regime, attribution)
...
| 編號 | 嚴重度 | 模組 | 說明 |
|------|--------|------|------|
| D-01 | **致命** | `strategy/engine.py` | `weights_to_orders()` 忽略合約乘數 (`multiplier`)，期貨數量計算錯誤 200~500 倍 |
| D-02 | **致命** | `domain/models.py` | `Portfolio.nav` 直接加總不同幣別市值（TWD+USD 以 1:1 計算），混幣回測結果無意義 |
| D-03 | **致命** | `backtest/engine.py` | 回測引擎假設單一幣別，不呼叫 `total_cash(fx_rates)` 也不載入匯率數據 |
| D-04 | **高** | `execution/sim.py` | SimBroker 手續費/稅為全域固定值，不使用 Instrument 的 per-instrument 費率 |
| D-05 | **高** | `alpha/strategy.py` | AlphaStrategy 只產出個股權重，無資產類別配置層（股/債ETF/商品/期貨的比例） |
| D-06 | **高** | `instrument/` | InstrumentRegistry 已建好但無任何現有模組消費它 |
| D-07 | **中** | `data/sources/yahoo.py` | 無重試/退避邏輯，大批量下載時 Yahoo 限流導致靜默失敗 |
| D-08 | **中** | `api/routes/alpha.py` | Pipeline 研究計算受 GIL 限制（ThreadPool 對 CPU 密集無效），應用 ProcessPool |
| D-09 | **低** | 前端 | 標的列表硬編碼在 TypeScript，未從後端 InstrumentRegistry 動態取得 |
...
| 數據源 | ✅ Yahoo + FinMind | 缺即時行情、FRED 宏觀數據 |
| 策略框架 | ✅ 完整 | 7 策略 + alpha + 插件式載入 |
| Alpha 研究 (資產內) | ✅ 完整 | 中性化/正交化/分位數/換手率/Pipeline/regime/attribution |
| 資產間配置 | ❌ 未實作 | 宏觀因子、跨資產信號、戰術配置 |
| 多資產組合最佳化 | ❌ 未實作 | Risk Parity, BL, 幣別對沖 |
| Instrument Registry | ❌ 未實作 | 統一標的模型 (合約規格/幣別/保證金) |
| 多幣別 Portfolio | ❌ 未實作 | 目前僅單幣別 |
...
| 差距 | 說明 |
|------|------|
| Instrument Registry | 無統一標的模型，無法表達期貨合約規格/債券票息/幣別 |
| 多幣別 Portfolio | 單一 `cash: Decimal`，無法管理多幣別資產 |
| 資產間配置層 | 無宏觀因子模型、無跨資產信號、無戰術配置引擎 |
| 多資產最佳化器 | 無 Risk Parity / Black-Litterman / HRP |
| 宏觀數據源 | 無 FRED (美國利率/GDP/CPI)、無台灣央行數據 |
...
### Phase A：多資產基礎設施 + 管線整合（當前目標）

✅ A1~A4 完成：Instrument Registry、多幣別欄位、DataFeed 擴展、FRED
⬜ A5 待完成：管線整合（weights_to_orders 乘數、SimBroker 費率、多幣別 NAV、回測引擎 FX）
⬜ A6 待完成：YahooFeed 重試/限流
