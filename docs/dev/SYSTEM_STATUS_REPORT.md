...
> **當前階段**: Phase D 進行中（系統整合 + 風控升級：MultiAssetStrategy + 跨資產風控規則）
> **代碼庫起始日期**: 2026-03-22
> **總提交次數**: 44 commits
> **當前分支**: master
...
| `src/portfolio/` | 4 | ~650 | 組合最佳化 (6 方法), 風險模型 (Ledoit-Wolf), 幣別對沖 |
...
| 分類 | 檔案 | 框架 |
|------|------|------|
| Python 單元/整合 | 40 | pytest (~7,500 LOC) |
...
### 4.3 組合最佳化層 (`src/portfolio/`) — Phase C NEW

| 檔案 | 功能 |
|------|------|
| `optimizer.py` | 6 種方法：Equal Weight, Inverse Vol, Risk Parity, MVO, Black-Litterman (含 views), HRP |
| `risk_model.py` | 共變異數矩陣（歷史法 + EWM + Ledoit-Wolf）、相關矩陣、風險貢獻分解 |
| `currency.py` | 幣別對沖引擎：分級策略（<10%/10~40%/>40%）、HedgeRecommendation |

### 4.4 策略引擎 (`src/strategy/`)
...
**後端**: 40 檔 — 含 9 個 Alpha + 1 個 Allocation + 1 個 Portfolio Optimizer 測試檔
...
| 多資產組合最佳化 | ✅ Phase C | 6 方法 (MVO/RP/BL/HRP/EW/IV) + 幣別對沖 + 風險模型 |
...
| ~~多資產最佳化器~~ | ✅ | 6 種方法 + 幣別對沖 + Ledoit-Wolf 風險模型 (Phase C) |
...
### Phase C：多資產組合最佳化 ✅ 已完成

✅ C1：多資產最佳化器 (`src/portfolio/optimizer.py`) — EW/IV/RP/MVO/BL/HRP 六種方法
✅ C2：幣別對沖 (`src/portfolio/currency.py`) — 分級對沖策略 + HedgeRecommendation
✅ C3：風險模型 (`src/portfolio/risk_model.py`) — 歷史法/EWM/Ledoit-Wolf + 風險貢獻分解

### Phase D：系統整合 + 風控升級（進行中）

✅ D1：MultiAssetStrategy (`src/strategy/multi_asset.py`) — 兩層配置策略，已註冊至 registry
✅ D2：跨資產風控規則 (`src/risk/rules.py`) — max_asset_class_weight, max_currency_exposure, max_gross_leverage
✅ Bug fixes：FX per-bar 更新、總權重驗證、FRED ffill 上限
