  ...
  return (
    <div className="bg-surface rounded-xl p-5">
      <div className="flex items-center justify-between mb-3">
        <p className="text-sm font-medium text-slate-400">{t.backtest.tradeDetail}</p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          ...
          <tbody>
            {trades.map((tr, i) => (
              <tr key={i} className="border-b border-surface-light/50 hover:bg-surface-light/30 transition-colors">
                <td className="px-3 py-2 text-slate-300">{tr.date}</td>
                <td className="px-3 py-2 text-slate-200 font-medium">{tr.symbol}</td>
                <td className={`px-3 py-2 font-medium ${tr.side === "BUY" ? "text-emerald-400" : "text-red-400"}`}>
                  {tr.side}
                </td>
                <td className="px-3 py-2 text-right text-slate-300 font-mono">{fmtNum(tr.quantity)}</td>
                <td className="px-3 py-2 text-right text-slate-300 font-mono">{fmtCurrency(tr.price)}</td>
                <td className="px-3 py-2 text-right text-slate-300 font-mono">{fmtCurrency(tr.commission)}</td>
              </tr>
            ))}
          </tbody>
        </table>
